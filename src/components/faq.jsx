...
function Faq({faqData}) {
  ...
  return (
    <>
      <section className={`section-padding bg-gray-100 ${faqData?.faqPoints?.length > 0 ? '' : 'hidden'}`}>
        <div className="container">
          <div className="mb-7">
            <h2 className="h2-title !mb-0 text-center" dangerouslySetInnerHTML={{__html: faqData?.faqTitle}}>
            </h2>
          </div>
          {/* accordion start */}
          <div className={`max-w-5xl mx-auto space-y-5`}>
            {faqData?.faqPoints?.map((faq, index) => (
              <div key={'faq-' + index} className={`[box-shadow:0_2px_10px_-3px_rgba(6,81,237,0.3)] border-l-8 border-primary/55 bg-white faq-${index}`}
                role="accordion"
              >
                <button
                  type="button"
                  className="accordion-button cursor-pointer w-full text-[15px] font-semibold text-left py-5 px-6 text-gray-800 flex items-center"
                ...
                >
                  ...
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className={`w-[14px] h-[14px] arrow fill-current ml-auto shrink-0 transition-all duration-300 ${index == activeIndex ? 'rotate-180' : ''}`}
                    viewBox="0 0 24 24"
                  >
                    <path
                      fillRule="evenodd"
                ...
                <div className={`accordion-content overflow-hidden transition-all duration-300 ease-in-out ${index == activeIndex ? '' : 'max-h-0'}`}>
            ...
            <div className={`text-center mt-12  ${isFaqPage ? 'hidden' : ''}`}>
              <Link href={faqData?.faqLink?.url ? faqData?.faqLink?.url : '#'} className="custom-button">
                {faqData?.faqLink?.title ? faqData?.faqLink?.title : 'View All FAQs'}
              </Link>
            </div>
          </div>
        </div>
